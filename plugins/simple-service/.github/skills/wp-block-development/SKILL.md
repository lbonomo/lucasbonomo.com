...
- designing/fixing spacing behavior tied to `supports.spacing` and `theme.json` spacing presets
...
### 8.5) Spacing and block theme integration (`theme.json` + block supports)

If spacing is part of the task, treat it as a cross-file concern:

1. In the target block, verify `supports.spacing` in `block.json` matches intended controls.
2. In the active theme, verify `settings.spacing` enables the controls and presets your block relies on.
3. Choose a spacing preset strategy:
   - `spacingScale` for generated mathematical scales.
   - `spacingSizes` for explicit fixed/fluid presets (`clamp()` etc.).
4. Validate `blockGap` mode intentionally (`null` vs `true` vs `false`) because UI and generated CSS behavior differ.
5. If you need contextual rhythm exceptions (for example heading transitions), use targeted CSS and verify editor + frontend parity.
6. If root horizontal padding is used with full-width blocks, validate `useRootPaddingAwareAlignments` behavior.

Read:
- `references/spacing-in-block-themes.md`
...
- Spacing controls, presets, and generated CSS behavior are correct across flow/flex/grid contexts when spacing was changed.
...
- `references/spacing-in-block-themes.md` (blockGap/presets/theme.json alignment issues)
